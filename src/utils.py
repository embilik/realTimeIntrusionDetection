...
def extract_source_data(spark: SparkSession, sources: List[dict]) -> dict:
    ...
    try:
        ...
        print(f"Error in extract_source_data: {str(e)}")
    return df_dict
...
def read_source_data(
    ...
    try:
        ...
        print(f"Error in read_source_data: {str(e)}")
    return None
...
def save_to_sink(df: DataFrame, config: dict, env: str = "prod") -> None:
    ...
    try:
        ...
        if config["storeType"] == "object_store":
            df.write.option("header", "true").format(sink_conf["format"]).mode(sink_conf["mode"]).save(
                sink_conf["path"].format(env=env)
            )
    except Exception as e:
        print(f"Error in save_to_sink: {str(e)}")
...
def read_job_config(config_file_name: str) -> dict:
    ...
    try:
        ...
        print(f"Error in read_job_config: {str(e)}")
    return None
...
def parse_known_cmd_args():
    ...
    try:
        ...
        print(f"Error in parse_known_cmd_args: {str(e)}")
        return None
